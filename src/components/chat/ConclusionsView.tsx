  ...
  return (
    ...
    >
      <motion.div variants={fadeUp} className="flex items-center gap-3">
        <div className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-[#1f86ff] to-[#8b6cff] text-white shadow-[0_10px_30px_-10px_rgba(31,134,255,0.6)]">
          <FiCheckCircle />
        </div>
        <div>
          <p className="mono text-[10px] uppercase tracking-[0.28em] text-[var(--brand-300)]">
            assessment complete
          </p>
          ...
          {safe.map((c) => (
            <motion.li key={c.id} variants={fadeUp}>
              <article className="group relative h-full rounded-2xl glass border border-white/10 p-5 overflow-hidden hover:border-white/25 transition-all duration-500">
                <div className="pointer-events-none absolute -top-20 -right-16 h-48 w-48 rounded-full bg-gradient-to-br from-sky-500/25 to-blue-700/0 blur-3xl opacity-60 group-hover:opacity-100 transition-opacity duration-500" />
...
function Metric({
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-full glass border border-white/10 px-3 py-1.5">
      <span className="text-[var(--brand-300)]">{icon}</span>
